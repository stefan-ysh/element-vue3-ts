<script setup lang="ts">
  import {
    ref,
    defineComponent,
    watch,
    getCurrentInstance,
    onMounted
  } from 'vue'
  const {
    proxy,
    ctx
  } = getCurrentInstance() as any

  // defineProps < {
  //   msg: string
  // } > ()
  // interface Elements {
  //   table:any
  //   lanthanoids:any
  //   actinoids:any
  // }
  let elementType: any = ref({
    table: [{
        elements: [{
            group: "Nonmetal",
            position: 0,
            name: "Hydrogen",
            number: 1,
            small: "H",
            molar: 1.00794,
            electrons: [1],
            CNname: "氢"
          },
          {},
          {
            isEmpty: true
          },
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {
            group: "Noble",
            position: 17,
            name: "Helium",
            number: 2,
            small: "He",
            molar: 4.002602,
            electrons: [2],
            CNname: "氦"
          }
        ]
      },
      {
        elements: [{
            group: "Alkali",
            position: 0,
            name: "Lithium",
            number: 3,
            small: "Li",
            molar: 6.941,
            electrons: [2, 1]
          },
          {
            group: "Alkaline",
            position: 1,
            name: "Beryllium",
            number: 4,
            small: "Be",
            molar: 9.012182,
            electrons: [2, 2]
          },
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {
            group: "Metalloid",
            position: 12,
            name: "Boron",
            number: 5,
            small: "B",
            molar: 10.811,
            electrons: [2, 3]
          },
          {
            group: "Nonmetal",
            position: 13,
            name: "Carbon",
            number: 6,
            small: "C",
            molar: 12.0107,
            electrons: [2, 4]
          },
          {
            group: "Nonmetal",
            position: 14,
            name: "Nitrogen",
            number: 7,
            small: "N",
            molar: 14.0067,
            electrons: [2, 5]
          },
          {
            group: "Nonmetal",
            position: 15,
            name: "Oxygen",
            number: 8,
            small: "O",
            molar: 15.9994,
            electrons: [2, 6]
          },
          {
            group: "Halogen",
            position: 16,
            name: "Fluorine",
            number: 9,
            small: "F",
            molar: 18.998404,
            electrons: [2, 7]
          },
          {
            group: "Noble",
            position: 17,
            name: "Neon",
            number: 10,
            small: "Ne",
            molar: 20.1797,
            electrons: [2, 8]
          }
        ]
      },
      {
        elements: [{
            group: "Alkali",
            position: 0,
            name: "Sodium",
            number: 11,
            small: "Na",
            molar: 22.989769,
            electrons: [2, 8, 1]
          },
          {
            group: "Alkaline",
            position: 1,
            name: "Magnesium",
            number: 12,
            small: "Mg",
            molar: 24.305,
            electrons: [2, 8, 2]
          },
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {},
          {
            group: "Poor",
            position: 12,
            name: "Aluminium",
            number: 13,
            small: "Al",
            molar: 26.981539,
            electrons: [2, 8, 3]
          },
          {
            group: "Metalloid",
            position: 13,
            name: "Silicon",
            number: 14,
            small: "Si",
            molar: 28.0855,
            electrons: [2, 8, 4]
          },
          {
            group: "Nonmetal",
            position: 14,
            name: "Phosphorus",
            number: 15,
            small: "P",
            molar: 30.973763,
            electrons: [2, 8, 5]
          },
          {
            group: "Nonmetal",
            position: 15,
            name: "Sulfur",
            number: 16,
            small: "S",
            molar: 32.065,
            electrons: [2, 8, 6]
          },
          {
            group: "Halogen",
            position: 16,
            name: "Chlorine",
            number: 17,
            small: "Cl",
            molar: 35.453,
            electrons: [2, 8, 7]
          },
          {
            group: "Noble",
            position: 17,
            name: "Argon",
            number: 18,
            small: "Ar",
            molar: 39.948,
            electrons: [2, 8, 8]
          }
        ]
      },
      {
        elements: [{
            group: "Alkali",
            position: 0,
            name: "Potassium",
            number: 19,
            small: "K",
            molar: 39.0983,
            electrons: [2, 8, 8, 1]
          },
          {
            group: "Alkaline",
            position: 1,
            name: "Calcium",
            number: 20,
            small: "Ca",
            molar: 40.078,
            electrons: [2, 8, 8, 2]
          },
          {
            group: "Transition ",
            position: 2,
            name: "Scandium",
            number: 21,
            small: "Sc",
            molar: 44.955914,
            electrons: [2, 8, 9, 2]
          },
          {
            group: "Transition ",
            position: 3,
            name: "Titanium",
            number: 22,
            small: "Ti",
            molar: 47.867,
            electrons: [2, 8, 10, 2]
          },
          {
            group: "Transition ",
            position: 4,
            name: "Vanadium",
            number: 23,
            small: "V",
            molar: 50.9415,
            electrons: [2, 8, 11, 2]
          },
          {
            group: "Transition ",
            position: 5,
            name: "Chromium",
            number: 24,
            small: "Cr",
            molar: 51.9961,
            electrons: [2, 8, 13, 1]
          },
          {
            group: "Transition ",
            position: 6,
            name: "Manganese",
            number: 25,
            small: "Mn",
            molar: 54.938046,
            electrons: [2, 8, 13, 2]
          },
          {
            group: "Transition ",
            position: 7,
            name: "Iron",
            number: 26,
            small: "Fe",
            molar: 55.845,
            electrons: [2, 8, 14, 2]
          },
          {
            group: "Transition ",
            position: 8,
            name: "Cobalt",
            number: 27,
            small: "Co",
            molar: 58.933193,
            electrons: [2, 8, 15, 2]
          },
          {
            group: "Transition ",
            position: 9,
            name: "Nickel",
            number: 28,
            small: "Ni",
            molar: 58.6934,
            electrons: [2, 8, 16, 2]
          },
          {
            group: "Transition ",
            position: 10,
            name: "Copper",
            number: 29,
            small: "Cu",
            molar: 63.546,
            electrons: [2, 8, 18, 1]
          },
          {
            group: "Transition ",
            position: 11,
            name: "Zinc",
            number: 30,
            small: "Zn",
            molar: 65.38,
            electrons: [2, 8, 18, 2]
          },
          {
            group: "Poor",
            position: 12,
            name: "Gallium",
            number: 31,
            small: "Ga",
            molar: 69.723,
            electrons: [2, 8, 18, 3]
          },
          {
            group: "Metalloid",
            position: 13,
            name: "Germanium",
            number: 32,
            small: "Ge",
            molar: 72.63,
            electrons: [2, 8, 18, 4]
          },
          {
            group: "Metalloid",
            position: 14,
            name: "Arsenic",
            number: 33,
            small: "As",
            molar: 74.9216,
            electrons: [2, 8, 18, 5]
          },
          {
            group: "Nonmetal",
            position: 15,
            name: "Selenium",
            number: 34,
            small: "Se",
            molar: 78.96,
            electrons: [2, 8, 18, 6]
          },
          {
            group: "Halogen",
            position: 16,
            name: "Bromine",
            number: 35,
            small: "Br",
            molar: 79.904,
            electrons: [2, 8, 18, 7]
          },
          {
            group: "Noble",
            position: 17,
            name: "Krypton",
            number: 36,
            small: "Kr",
            molar: 83.798,
            electrons: [2, 8, 18, 8]
          }
        ]
      },
      {
        elements: [{
            group: "Alkali",
            position: 0,
            name: "Rubidium",
            number: 37,
            small: "Rb",
            molar: 85.4678,
            electrons: [2, 8, 18, 8, 1]
          },
          {
            group: "Alkaline",
            position: 1,
            name: "Strontium",
            number: 38,
            small: "Sr",
            molar: 87.62,
            electrons: [2, 8, 18, 8, 2]
          },
          {
            group: "Transition ",
            position: 2,
            name: "Yttrium",
            number: 39,
            small: "Y",
            molar: 88.90585,
            electrons: [2, 8, 18, 9, 2]
          },
          {
            group: "Transition ",
            position: 3,
            name: "Zirconium",
            number: 40,
            small: "Zr",
            molar: 91.224,
            electrons: [2, 8, 18, 10, 2]
          },
          {
            group: "Transition ",
            position: 4,
            name: "Niobium",
            number: 41,
            small: "Nb",
            molar: 92.90638,
            electrons: [2, 8, 18, 12, 1]
          },
          {
            group: "Transition ",
            position: 5,
            name: "Molybdenum",
            number: 42,
            small: "Mo",
            molar: 95.96,
            electrons: [2, 8, 18, 13, 1]
          },
          {
            group: "Transition ",
            position: 6,
            name: "Technetium",
            number: 43,
            small: "Tc",
            molar: 98,
            electrons: [2, 8, 18, 13, 2]
          },
          {
            group: "Transition ",
            position: 7,
            name: "Ruthenium",
            number: 44,
            small: "Ru",
            molar: 101.07,
            electrons: [2, 8, 18, 15, 1]
          },
          {
            group: "Transition ",
            position: 8,
            name: "Rhodium",
            number: 45,
            small: "Rh",
            molar: 102.9055,
            electrons: [2, 8, 18, 16, 1]
          },
          {
            group: "Transition ",
            position: 9,
            name: "Palladium",
            number: 46,
            small: "Pd",
            molar: 106.42,
            electrons: [2, 8, 18, 18]
          },
          {
            group: "Transition ",
            position: 10,
            name: "Silver",
            number: 47,
            small: "Ag",
            molar: 107.8682,
            electrons: [2, 8, 18, 18, 1]
          },
          {
            group: "Transition ",
            position: 11,
            name: "Cadmium",
            number: 48,
            small: "Cd",
            molar: 112.411,
            electrons: [2, 8, 18, 18, 2]
          },
          {
            group: "Poor",
            position: 12,
            name: "Indium",
            number: 49,
            small: "In",
            molar: 114.818,
            electrons: [2, 8, 18, 18, 3]
          },
          {
            group: "Poor",
            position: 13,
            name: "Tin",
            number: 50,
            small: "Sn",
            molar: 118.71,
            electrons: [2, 8, 18, 18, 4]
          },
          {
            group: "Metalloid",
            position: 14,
            name: "Antimony",
            number: 51,
            small: "Sb",
            molar: 121.76,
            electrons: [2, 8, 18, 18, 5]
          },
          {
            group: "Metalloid",
            position: 15,
            name: "Tellurium",
            number: 52,
            small: "Te",
            molar: 127.6,
            electrons: [2, 8, 18, 18, 6]
          },
          {
            group: "Halogen",
            position: 16,
            name: "Iodine",
            number: 53,
            small: "I",
            molar: 126.90447,
            electrons: [2, 8, 18, 18, 7]
          },
          {
            group: "Noble",
            position: 17,
            name: "Xenon",
            number: 54,
            small: "Xe",
            molar: 131.293,
            electrons: [2, 8, 18, 18, 8]
          }
        ]
      },
      {
        elements: [{
            group: "Alkali",
            position: 0,
            name: "Caesium",
            number: 55,
            small: "Cs",
            molar: 132.90546,
            electrons: [2, 8, 18, 18, 8, 1]
          },
          {
            group: "Alkaline",
            position: 1,
            name: "Barium",
            number: 56,
            small: "Ba",
            molar: 137.327,
            electrons: [2, 8, 18, 18, 8, 2]
          },
          {
            group: "Lanthanoid",
            position: 2,
            name: "",
            number: "",
            small: "57-71"
          },
          {
            group: "Transition ",
            position: 3,
            name: "Hafnium",
            number: 72,
            small: "Hf",
            molar: 178.49,
            electrons: [2, 8, 18, 32, 10, 2]
          },
          {
            group: "Transition ",
            position: 4,
            name: "Tantalum",
            number: 73,
            small: "Ta",
            molar: 180.94788,
            electrons: [2, 8, 18, 32, 11, 2]
          },
          {
            group: "Transition ",
            position: 5,
            name: "Tungsten",
            number: 74,
            small: "W",
            molar: 183.84,
            electrons: [2, 8, 18, 32, 12, 2]
          },
          {
            group: "Transition ",
            position: 6,
            name: "Rhenium",
            number: 75,
            small: "Re",
            molar: 186.207,
            electrons: [2, 8, 18, 32, 13, 2]
          },
          {
            group: "Transition ",
            position: 7,
            name: "Osmium",
            number: 76,
            small: "Os",
            molar: 190.23,
            electrons: [2, 8, 18, 32, 14, 2]
          },
          {
            group: "Transition ",
            position: 8,
            name: "Iridium",
            number: 77,
            small: "Ir",
            molar: 192.217,
            electrons: [2, 8, 18, 32, 15, 2]
          },
          {
            group: "Transition ",
            position: 9,
            name: "Platinum",
            number: 78,
            small: "Pt",
            molar: 195.084,
            electrons: [2, 8, 18, 32, 17, 1]
          },
          {
            group: "Transition ",
            position: 10,
            name: "Gold",
            number: 79,
            small: "Au",
            molar: 196.96657,
            electrons: [2, 8, 18, 32, 18, 1]
          },
          {
            group: "Transition ",
            position: 11,
            name: "Mercury",
            number: 80,
            small: "Hg",
            molar: 200.59,
            electrons: [2, 8, 18, 32, 18, 2]
          },
          {
            group: "Poor",
            position: 12,
            name: "Thallium",
            number: 81,
            small: "Tl",
            molar: 204.3833,
            electrons: [2, 8, 18, 32, 18, 3]
          },
          {
            group: "Poor",
            position: 13,
            name: "Lead",
            number: 82,
            small: "Pb",
            molar: 207.2,
            electrons: [2, 8, 18, 32, 18, 4]
          },
          {
            group: "Poor",
            position: 14,
            name: "Bismuth",
            number: 83,
            small: "Bi",
            molar: 208.9804,
            electrons: [2, 8, 18, 32, 18, 5]
          },
          {
            group: "Metalloid",
            position: 15,
            name: "Polonium",
            number: 84,
            small: "Po",
            molar: 209,
            electrons: [2, 8, 18, 32, 18, 6]
          },
          {
            group: "Halogen",
            position: 16,
            name: "Astatine",
            number: 85,
            small: "At",
            molar: 210,
            electrons: [2, 8, 18, 32, 18, 7]
          },
          {
            group: "Noble",
            position: 17,
            name: "Radon",
            number: 86,
            small: "Rn",
            molar: 222,
            electrons: [2, 8, 18, 32, 18, 8]
          }
        ]
      },
      {
        elements: [{
            group: "Alkali",
            position: 0,
            name: "Francium",
            number: 87,
            small: "Fr",
            molar: 223,
            electrons: [2, 8, 18, 32, 18, 8, 1]
          },
          {
            group: "Alkaline",
            position: 1,
            name: "Radium",
            number: 88,
            small: "Ra",
            molar: 226,
            electrons: [2, 8, 18, 32, 18, 8, 2]
          },
          {
            group: "Actinoid",
            position: 2,
            name: "",
            number: "",
            small: "89-103"
          },
          {
            group: "Transition ",
            position: 3,
            name: "Rutherfordium",
            number: 104,
            small: "Rf",
            molar: 267,
            electrons: [2, 8, 18, 32, 32, 10, 2]
          },
          {
            group: "Transition ",
            position: 4,
            name: "Dubnium",
            number: 105,
            small: "Db",
            molar: 268,
            electrons: [2, 8, 18, 32, 32, 11, 2]
          },
          {
            group: "Transition ",
            position: 5,
            name: "Seaborgium",
            number: 106,
            small: "Sg",
            molar: 271,
            electrons: [2, 8, 18, 32, 32, 12, 2]
          },
          {
            group: "Transition ",
            position: 6,
            name: "Bohrium",
            number: 107,
            small: "Bh",
            molar: 272,
            electrons: [2, 8, 18, 32, 32, 13, 2]
          },
          {
            group: "Transition ",
            position: 7,
            name: "Hassium",
            number: 108,
            small: "Hs",
            molar: 270,
            electrons: [2, 8, 18, 32, 32, 14, 2]
          },
          {
            group: "Transition ",
            position: 8,
            name: "Meitnerium",
            number: 109,
            small: "Mt",
            molar: 276,
            electrons: [2, 8, 18, 32, 32, 15, 2]
          },
          {
            group: "Transition ",
            position: 9,
            name: "Darmstadtium",
            number: 110,
            small: "Ds",
            molar: 281,
            electrons: [2, 8, 18, 32, 32, 17, 1]
          },
          {
            group: "Transition ",
            position: 10,
            name: "Roentgenium",
            number: 111,
            small: "Rg",
            molar: 280,
            electrons: [2, 8, 18, 32, 32, 18, 1]
          },
          {
            group: "Transition ",
            position: 11,
            name: "Copernicium",
            number: 112,
            small: "Cn",
            molar: 285,
            electrons: [2, 8, 18, 32, 32, 18, 2]
          },
          {
            group: "Poor",
            position: 12,
            name: "Ununtrium",
            number: 113,
            small: "Uut",
            molar: 284,
            electrons: [2, 8, 18, 32, 32, 18, 3]
          },
          {
            group: "Poor",
            position: 13,
            name: "Flerovium",
            number: 114,
            small: "Fl",
            molar: 289,
            electrons: [2, 8, 18, 32, 32, 18, 4]
          },
          {
            group: "Poor",
            position: 14,
            name: "Ununpentium",
            number: 115,
            small: "Uup",
            molar: 288,
            electrons: [2, 8, 18, 32, 32, 18, 5]
          },
          {
            group: "Poor",
            position: 15,
            name: "Livermorium",
            number: 116,
            small: "Lv",
            molar: 293,
            electrons: [2, 8, 18, 32, 32, 18, 6]
          },
          {
            group: "Halogen",
            position: 16,
            name: "Ununseptium",
            number: 117,
            small: "Uus",
            molar: 294,
            electrons: [2, 8, 18, 32, 32, 18, 7]
          },
          {
            group: "Noble",
            position: 17,
            name: "Ununoctium",
            number: 118,
            small: "Uuo",
            molar: 294,
            electrons: [2, 8, 18, 32, 32, 18, 8]
          }
        ]
      }
    ],
    lanthanoids: [{
        group: "Lanthanoid",
        position: 2,
        name: "Lanthanum",
        number: 57,
        small: "La",
        molar: 138.90547,
        electrons: [2, 8, 18, 18, 9, 2]
      },
      {
        group: "Lanthanoid",
        position: 3,
        name: "Cerium",
        number: 58,
        small: "Ce",
        molar: 140.116,
        electrons: [2, 8, 18, 19, 9, 2]
      },
      {
        group: "Lanthanoid",
        position: 4,
        name: "Praseodymium",
        number: 59,
        small: "Pr",
        molar: 140.90765,
        electrons: [2, 8, 18, 21, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 5,
        name: "Neodymium",
        number: 60,
        small: "Nd",
        molar: 144.242,
        electrons: [2, 8, 18, 22, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 6,
        name: "Promethium",
        number: 61,
        small: "Pm",
        molar: 145,
        electrons: [2, 8, 18, 23, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 7,
        name: "Samarium",
        number: 62,
        small: "Sm",
        molar: 150.36,
        electrons: [2, 8, 18, 24, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 8,
        name: "Europium",
        number: 63,
        small: "Eu",
        molar: 151.964,
        electrons: [2, 8, 18, 25, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 9,
        name: "Gadolinium",
        number: 64,
        small: "Gd",
        molar: 157.25,
        electrons: [2, 8, 18, 25, 9, 2]
      },
      {
        group: "Lanthanoid",
        position: 10,
        name: "Terbium",
        number: 65,
        small: "Tb",
        molar: 158.92535,
        electrons: [2, 8, 18, 27, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 11,
        name: "Dysprosium",
        number: 66,
        small: "Dy",
        molar: 162.5,
        electrons: [2, 8, 18, 28, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 12,
        name: "Holmium",
        number: 67,
        small: "Ho",
        molar: 164.93031,
        electrons: [2, 8, 18, 29, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 13,
        name: "Erbium",
        number: 68,
        small: "Er",
        molar: 167.259,
        electrons: [2, 8, 18, 30, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 14,
        name: "Thulium",
        number: 69,
        small: "Tm",
        molar: 168.9342,
        electrons: [2, 8, 18, 31, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 15,
        name: "Ytterbium",
        number: 70,
        small: "Yb",
        molar: 173.054,
        electrons: [2, 8, 18, 32, 8, 2]
      },
      {
        group: "Lanthanoid",
        position: 16,
        name: "Lutetium",
        number: 71,
        small: "Lu",
        molar: 174.9668,
        electrons: [2, 8, 18, 32, 9, 2]
      }
    ],
    actinoids: [{
        group: "Actinoid",
        position: 2,
        name: "Actinium",
        number: 89,
        small: "Ac",
        molar: 227,
        electrons: [2, 8, 18, 32, 18, 9, 2]
      },
      {
        group: "Actinoid",
        position: 3,
        name: "Thorium",
        number: 90,
        small: "Th",
        molar: 232.03806,
        electrons: [2, 8, 18, 32, 18, 10, 2]
      },
      {
        group: "Actinoid",
        position: 4,
        name: "Protactinium",
        number: 91,
        small: "Pa",
        molar: 231.03587,
        electrons: [2, 8, 18, 32, 20, 9, 2]
      },
      {
        group: "Actinoid",
        position: 5,
        name: "Uranium",
        number: 92,
        small: "U",
        molar: 238.02892,
        electrons: [2, 8, 18, 32, 21, 9, 2]
      },
      {
        group: "Actinoid",
        position: 6,
        name: "Neptunium",
        number: 93,
        small: "Np",
        molar: 237,
        electrons: [2, 8, 18, 32, 22, 9, 2]
      },
      {
        group: "Actinoid",
        position: 7,
        name: "Plutonium",
        number: 94,
        small: "Pu",
        molar: 244,
        electrons: [2, 8, 18, 32, 24, 8, 2]
      },
      {
        group: "Actinoid",
        position: 8,
        name: "Americium",
        number: 95,
        small: "Am",
        molar: 243,
        electrons: [2, 8, 18, 32, 25, 8, 2]
      },
      {
        group: "Actinoid",
        position: 9,
        name: "Curium",
        number: 96,
        small: "Cm",
        molar: 247,
        electrons: [2, 8, 18, 32, 25, 9, 2]
      },
      {
        group: "Actinoid",
        position: 10,
        name: "Berkelium",
        number: 97,
        small: "Bk",
        molar: 247,
        electrons: [2, 8, 18, 32, 27, 8, 2]
      },
      {
        group: "Actinoid",
        position: 11,
        name: "Californium",
        number: 98,
        small: "Cf",
        molar: 251,
        electrons: [2, 8, 18, 32, 28, 8, 2]
      },
      {
        group: "Actinoid",
        position: 12,
        name: "Einsteinium",
        number: 99,
        small: "Es",
        molar: 252,
        electrons: [2, 8, 18, 32, 29, 8, 2]
      },
      {
        group: "Actinoid",
        position: 13,
        name: "Fermium",
        number: 100,
        small: "Fm",
        molar: 257,
        electrons: [2, 8, 18, 32, 30, 8, 2]
      },
      {
        group: "Actinoid",
        position: 14,
        name: "Mendelevium",
        number: 101,
        small: "Md",
        molar: 258,
        electrons: [2, 8, 18, 32, 31, 8, 2]
      },
      {
        group: "Actinoid",
        position: 15,
        name: "Nobelium",
        number: 102,
        small: "No",
        molar: 259,
        electrons: [2, 8, 18, 32, 32, 8, 2]
      },
      {
        group: "Actinoid",
        position: 16,
        name: "Lawrencium",
        number: 103,
        small: "Lr",
        molar: 262,
        electrons: [2, 8, 18, 32, 32, 8, 3]
      }
    ]
  })

  let zoom = ref(1)

  let currentEleObj = ref([])

  let result = ref(0)

  let curEle = ref({}) as any

  let curGroup = ref('')

  let curEleArr = ref([]) as any

  const seeDetail = (ele: any) => {
    proxy.curEle = ele;
    proxy.curGroup = ele.group;
  }
  const computeScreenWidthHeight = () => {
    // 高度计算zoom
    // var percentage,
    //   appHeight = 768; //设置默认屏幕大小
    // percentage = (window.innerHeight * 100) / appHeight;
    // percentage = Math.floor(percentage) / 100;
    // 宽度计算
    var percentageWidth, clientWidth = 1700;
    percentageWidth = document.body.clientWidth * 100 / clientWidth;
    percentageWidth = Math.floor(percentageWidth) / 100;
    proxy.zoom = percentageWidth;
  }

  const onMouseout = () => {
    proxy.curGroup = "";
    proxy.curEle = "";
  }
  const addEle = (item: any) => {
    if (item.molar) {
      proxy.currentEleObj.push(item);
    }
  }

  const resetResult = () => {
    proxy.currentEleObj = [];
  }

  const copyResult = () => {
    proxy.currentEleObj = [];
  }

  const popEle = () => {
    proxy.currentEleObj.pop();
  }

  onMounted(() => {
    proxy.computeScreenWidthHeight();
    window.addEventListener("resize", function () {
      //要执行的动作
      proxy.computeScreenWidthHeight();
    });
  })

  //监视---监视指定的数据
  /*
    第一个值监听对象，第二个值监听的回调函数，第三个值: watch属性(immediate,deep)
  */
  watch(currentEleObj, (ele) => {
    proxy.curEleArr = [];
    proxy.result = 0;
    ele.forEach((item: any) => {
      proxy.curEleArr.push(item.small);
      proxy.result += item.molar;
    });
  }, {
    immediate: true,
    deep: true
  })
</script>

<template>
  <div class="element-wrap" :style="{ zoom: zoom }">
    <div>
      <!-- 正常元素 -->
      <span class="col table" v-for="(item, index) in elementType.table" :key="index">
        <span v-for="(oItem, oIndex) in item.elements" :key="oIndex" :class="
            `table-ele element ${oItem.group ? oItem.group : ''} ${
              curGroup && oItem.group !== curGroup && !oItem.isEmpty
                ? 'opacity-item'
                : ''
            }`
          " @click="addEle(oItem)" @mouseover="seeDetail(oItem)" @mouseleave="onMouseout()" :style="
            !oItem.small && oItem.isEmpty
              ? 'position:relative;'
              : oItem.small
              ? ''
              : 'visibility: hidden;'
          ">
          <!-- 详细信息框 -->
          <div v-if="!oItem.small && oItem.isEmpty" class="ele-detail">
            <!--  -->
            <div class="ele-detail-top">
              <div class="detail">
                <span v-if="curEle.number">Number: {{ curEle.number }}<br /></span>
                <span v-if="curEle.number">Name: {{ curEle.small }}<br /></span>
                <span v-if="curEle.number">Molar: {{ curEle.molar }}<br /></span>
                <span v-if="curEle.group">Group: {{ curEle.group }}<br /></span>
              </div>
              <div class="solarsys" v-if="curEle.electrons" :style="
                  curEle.electrons.length >= 4 && curEle.electrons.length < 7
                    ? 'transform: scale(0.7);'
                    : curEle.electrons.length >= 7
                    ? 'transform: scale(0.6);'
                    : ''
                ">
                <!--中心电子数量-->
                <div class="sun">
                  +
                  {{
                    curEle.electrons.reduce((a:any, b:any) =>{
                      return a + b;
                    }, 0)
                  }}
                </div>
                <span v-for="(item, index) in curEle.electrons" :key="index">
                  <!--电子轨道-->
                  <div class="mercuryOrbit" :style="
                      `margin-top: ${50 -
                        (index + 1) * 25}px; margin-left: ${50 -
                        (index + 1) * 25}px;height: ${(index + 1) *
                        50}px;width: ${(index + 1) * 50}px;left:${(index + 1) *
                        50};top:${(index + 1) * 50};`
                    "></div>
                  <span v-for="(oItem, oIndex) in item" :key="oIndex">
                    <!--电子-->
                    <div class="mercury" :style="
                        ` left: ${20 -
                          index *
                            25}px; animation-duration:${(oIndex +
                          1) *
                          Math.floor(
                            Math.random() * (8 - 1 + 4) + 4
                          )}s;transform-origin: ${30 +
                          index *
                            25}px;background-color: rgb(${index}66, ${index}38, ${index}5);`
                      "></div>
                  </span>
                </span>
              </div>
            </div>
            <!-- 计算 -->
            <span class="ele-detail-bottom">
              <p>{{ curEleArr.join(" + ") }}</p>
              <span>{{ result ? result.toFixed(3) : "" }}</span>
              <!-- <a-button v-if="result" size="small" @click="copyResult">复制</a-button> -->
              <span class="btn del-btn" v-if="result" @click="popEle">Del</span>
              <span class="btn clear-btn" v-if="result" @click="resetResult">Clear</span>
            </span>
          </div>
          <span class="number">{{ oItem.number }}</span>
          <span class="name">{{ oItem.small }}</span>
          <span class="molar">{{
            oItem.molar ? oItem.molar.toFixed(3) : oItem.molar
          }}</span>
        </span>
      </span>
      <!-- 镧系元素 -->
      <span class="col lanthanoids">
        <span v-for="(item, index) in elementType.lanthanoids" :key="index" :class="
            `lanthanoids-ele element ${item.group ? item.group : ''} ${
              curGroup && item.group !== curGroup ? 'opacity-item' : ''
            }`
          " @click="addEle(item)" @mouseover="seeDetail(item)" @mouseleave="onMouseout()">
          <span class="number">{{ item.number }}</span>
          <span class="name">{{ item.small }}</span>
          <span class="molar">{{
            item.molar ? item.molar.toFixed(3) : item.molar
          }}</span>
        </span>
      </span>
      <!-- 锕系元素 -->
      <span class="col actinoids">
        <span :class="
            `actinoids-ele element ${item.group ? item.group : ''} ${
              curGroup && item.group !== curGroup ? 'opacity-item' : ''
            }`
          " v-for="(item, index) in elementType.actinoids" :key="index" @click="addEle(item)"
          @mouseover="seeDetail(item)" @mouseleave="onMouseout()">
          <span class="number">{{ item.number }}</span>
          <span class="name">{{ item.small }}</span>
          <span class="molar">{{
            item.molar ? item.molar.toFixed(3) : item.molar
          }}</span>
        </span>
      </span>
    </div>
  </div>
</template>

<style lang="less" scoped>
  .element-wrap {
    position: relative;
    transition: all 5s;
    display: flex;
    justify-content: center;

    .col {
      display: flex;

      .element {
        min-width: 70px;
        min-height: 70px;
        width: 70px;
        height: 70px;
        display: flex;
        flex-direction: column;
        text-align: center;
        text-decoration: none;
        transition: all 0.3s ease-in-out;
        padding: 1px;
        cursor: pointer;
        border-radius: 10px;
        margin: 5px;
        // 阴影
        // box-shadow: inset 0 0 3px 0 rgba(156, 140, 228, 0.5);
        // background: -webkit-linear-gradient(top left, #26c6da, #00bcd4);
        color: #fff;

        // font-family: 'Roboto', sans-serif;
        &.Alkali {
          background: -webkit-linear-gradient(top left, #ef5350, #f44336);
        }

        &.Alkaline {
          background: -webkit-linear-gradient(top left, #ff7043, #ff5722);
        }

        &.Metalloid {
          background: -webkit-linear-gradient(top left, #26c6da, #00bcd4);
        }

        &.Nonmetal {
          background: -webkit-linear-gradient(top left, #42a5f5, #2196f3);
        }

        &.Halogen {
          background: -webkit-linear-gradient(top left, #5c6bc0, #3f51b5);
        }

        &.Noble {
          background: -webkit-linear-gradient(top left, #7e57c2, #673ab7);
        }

        &.Lanthanoid {
          // 镧系元素
          background: -webkit-linear-gradient(top left, #7e57c2, #673ab7);
        }

        &.Actinoid {
          // 锕系元素
          background: -webkit-linear-gradient(top left, #ab47bc, #9c27b0);
        }

        &.Poor {
          background: -webkit-linear-gradient(top left, #66bb6a, #4caf50);
        }

        &.Transition {
          background: -webkit-linear-gradient(top left, #ff9800, #fb8c00);
        }
      }

      .opacity-item {
        opacity: 0.25;
      }

      &.lanthanoids,
      &.actinoids {
        margin-top: 5px;
        margin-left: 140px;
      }

      .number {
        text-align: left;
      }

      .name {
        font-size: 160%;
        line-height: 30px;
        text-shadow: 1px 1px 2px rgb(122 13 120);
      }

      .molar {
        font-size: 10px;
      }
    }

    .table {
      .ele-detail {
        padding-top: 30px;
        position: absolute;
        width: 800px;
        height: 210px;
        background: #1e2f83;
        display: flex;
        justify-content: space-between;
        flex-direction: column;
        top: 0px;
        left: 5px;
        overflow: auto;

        &-top {
          display: flex;
          justify-content: space-between;

          align-items: center;

          .detail {
            text-align: left;
          }

          .solarsys {
            // width: 800px;
            // height: 800px;
            position: relative;
            margin: 30px 150px 0 0;
            // margin: 35px 135px;
            // background-color: #000000;
            padding: 0;

            .sun {
              left: 35px;
              top: 35px;
              height: 30px;
              width: 30px;
              line-height: 30px;
              background-color: rgb(248, 107, 35);
              border-radius: 50%;
              box-shadow: 5px 5px 10px rgb(248, 107, 35), -5px -5px 10px rgb(248, 107, 35),
                5px -5px 10px rgb(248, 107, 35), -5px 5px 10px rgb(248, 107, 35);
              position: absolute;
              margin: 0;
            }

            /*水星*/
            .mercury {
              left: 0;
              width: 10px;
              border-radius: 50%;
              position: absolute;
              animation: rotate 1.5s infinite linear;
              top: 45px;
              height: 10px;
              width: 10px;
            }

            /*水星轨道*/
            .mercuryOrbit {
              background-color: transparent;
              border-radius: 50%;
              border-style: dashed;
              border-color: gray;
              position: absolute;
              border-width: 1px;
              margin: 0px;
              padding: 0px;
            }

            @keyframes rotate {
              100% {
                transform: rotate(-360deg);
              }
            }
          }
        }

        &-bottom {
          .btn {
            cursor: pointer;
          }

          .del-btn {
            margin: 0 5px 0 20px;
          }

          .clear-btn {}
        }

      }
    }

  }
</style>